...
func newTestServerAndNode(t *testing.T, ns mock.Namesys) (*httptest.Server, API, context.Context) {
	...
	return ts, &a, context.Background()
}
...
func TestUriQueryRedirect(t *testing.T) {
	ts, _, _ := newTestServerAndNode(t, make(mock.Namesys))
...
var basicLinkProto = cidlink.LinkPrototype{
	Prefix: cid.NewPrefixV1(uint64(multicodec.DagJson), multihash.SHA2_256),
}

func TestGatewayGet(t *testing.T) {
	ns := make(mock.Namesys)
	ts, api, ctx := newTestServerAndNode(t, ns)

	ls := api.NewSession(ctx)
	l, err := ls.Store(ipld.LinkContext{}, basicLinkProto, basicnode.NewBytes([]byte("fnord")))
	if err != nil {
		t.Fatal(err)
	}
	...
	for i, test := range []struct {
		...
		{"127.0.0.1:8080", "/" + k.String(), http.StatusNotFound, "404 page not found\n"},
		{"127.0.0.1:8080", "/ipfs/" + k.String(), http.StatusOK, "fnord"},
		{"127.0.0.1:8080", "/ipns/nxdomain.example.com", http.StatusNotFound, "ipfs resolve -r /ipns/nxdomain.example.com: " + errors.New("could not resolve name").Error() + "\n"},
		{"127.0.0.1:8080", "/ipns/%0D%0A%0D%0Ahello", http.StatusNotFound, "ipfs resolve -r /ipns/\\r\\n\\r\\nhello: " + errors.New("could not resolve name").Error() + "\n"},
		...
		{"working.example.com", "/" + k.String(), http.StatusNotFound, "ipfs resolve -r /ipns/working.example.com" + k.String() + ": no link named \"ipfs\" under " + k.String() + "\n"},
		{"broken.example.com", "/", http.StatusNotFound, "ipfs resolve -r /ipns/broken.example.com/: " + errors.New("could not resolve name").Error() + "\n"},
		{"broken.example.com", "/" + k.String(), http.StatusNotFound, "ipfs resolve -r /ipns/broken.example.com" + k.String() + ": " + errors.New("could not resolve name").Error() + "\n"},
...
func TestPretty404(t *testing.T) {
	...
	ls := api.NewSession(ctx)
	var k cid.Cid
	if err := quickbuilder.Store(ls, func(b *quickbuilder.Builder) error {
		n := b.NewMapDirectory(map[string]quickbuilder.Node{
			"ipfs-404.html": b.NewBytesFile([]byte("Custom 404")),
			"deeper": b.NewMapDirectory(map[string]quickbuilder.Node{
				"ipfs-404.html": b.NewBytesFile([]byte("Deep custom 404")),
			}),
		})
		k = n.Link().(cidlink.Link).Cid
		return nil
	}); err != nil {
		t.Fatal(err)
	}
...
func TestIPNSHostnameRedirect(t *testing.T) {
	...
	if err := quickbuilder.Store(ls, func(b *quickbuilder.Builder) error {
		n := b.NewMapDirectory(map[string]quickbuilder.Node{
			"_": b.NewBytesFile([]byte("_")),
			"foo": b.NewMapDirectory(map[string]quickbuilder.Node{
				"index.html": b.NewBytesFile([]byte("_")),
			}),
		})
		k = n.Link().(cidlink.Link).Cid
		return nil
...
func TestIPNSHostnameBacklinks(t *testing.T) {
	...
	k2, err := api.Resolve(ctx, path.Join(k.String(), "foo? #<'"))
	if err != nil {
		t.Fatal(err)
	}
	k2c := NewPath(k2)

	k3, err := api.Resolve(ctx, path.Join(k.String(), "foo? #<'/bar"))
	if err != nil {
		t.Fatal(err)
	}
	k3c := NewPath(k3)
	...
	if !strings.Contains(s, k2c.String()) {
	...
	if !strings.Contains(s, k3c.String()) {
		t.Fatalf("expected hash in directory listing")
	}
}
